...
const props = defineProps({isResponse: Boolean, videos: Array})
...
<template>
  ...
  <div v-else v-for="(video, index) in videos"
       class="group p-2 overflow-hidden rounded-2xl bg-zinc-50 border border-zinc-200 cursor-pointer">
    <div v-show="!video.isHover"
         ...
         @mouseenter="playVideo(video, $refs.video[index])"><p
        class="absolute text-sm bottom-0 right-0 m-1 p-0.5 rounded-sm text-white font-medium bg-black/50 mb-1.5">
      {{ video.time }}</p></div>
    <div class="relative" v-show="video.isHover" @mouseleave="mouseleaveVideo(video, $refs.video[index])">
      ...
      <input type="range" @input="changeRange($event, video, $refs.video[index])"
             class="[&::-webkit-slider-thumb]:hidden hover:[&::-webkit-slider-thumb]:block h-1 z-1 accent-red-500 outline-none [&::-webkit-slider-thumb]:shadow-orange-400 shadow-orange-400 bg-black/20 appearance-none rounded-none cursor-pointer absolute bottom-0 w-full"
      ...
      <p class="absolute text-sm bottom-0 right-0 m-1 p-0.5 rounded-sm text-white font-medium bg-black/50 mb-1.5">
        {{ video.time }}</p></div>
    <div class="pt-3 flex gap-3 relative">
      <RouterLink v-if="!route.name.includes('channel')" :to="'/channel/' + video.user.id"
                  class="shrink-0 hidden sm:block"><img class="rounded-full w-10 h-10 object-cover"
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex flex-wrap gap-2 mt-2"><span class="px-2 py-0.5 text-xs rounded-full bg-blue-100 text-blue-700"> {{
            video.category?.name
          }} </span> <span class="px-2 py-0.5 text-xs rounded-full"
                           :class="video.public ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'"> {{
            video.public ? 'Публичное' : 'Приватное'
          }} </span></div>
