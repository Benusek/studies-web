...
onMounted(async () => {
  ...
  const result3 = await apiFetch('GET', `/video/${videoId}/comment`)

  if (result3.comments) {
    comments.value = result3.comments
      ...
      <div class="flex justify-between items-center p-3">
        ...
        <div class="flex flex-row gap-3 items-center">
          <div @click="getVideoPlaylistModal(video)"
            class="text-xs bg-white text-gray-500 px-2 py-1 rounded-lg select-none cursor-pointer border border-gray-400">
            Добавить в плейлист
          </div>
        </div>
        ...
        <div v-for="comment in comments">
          ...
          <div v-if="comment.isAnswer" class="flex flex-row items-center leading-none flex gap-2 ms-2 mt-2">
            <img class="rounded-full w-4 aspect-square"
                 :src="photo ? 'http://videoapi/'+ photo : '/src/assets/default.png' "
                 alt="user">
            <input type="text" placeholder="Оставить ответ..."
                   class=" border-b border-gray-200 text-sm p-1 w-full focus:border-white">
            <div
              class="border text-xs py-1 px-2 rounded-lg cursor-pointer border-gray-400 text-gray-500 select-none hover:bg-gray-100 active:bg-gray-200">
              Отправить
          ...
          <div v-if="comment.answers && comment.openAnswers" class="ms-5 flex flex-row"
            ...
            <div class="flex gap-2 p-2 items-center">
              ...
              <div>
                ...
                <span class="text-xs">{{ answer.text }}</span>
              </div>
            </div>
